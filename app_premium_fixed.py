...
# Professional CSS (same as before, keeping it)
st.markdown("""
<style>
...
"""
    ...
    if status_text:
        status_text.text("🧠 KI verarbeitet Daten...")
...
<div class="app-header">
    ...
    <div class="app-subtitle">Professionelle KI-gestützte Hotel-Analyse mit OpenAI GPT-4 • Echtzeit-Insights</div>
</div>
""", unsafe_allow_html=True)
            ...
            if pid:
                ...
                if not df_reviews.empty:
                    ...
                    if analysis:
                        ...
                        st.error("❌ KI-Analyse fehlgeschlagen. Bitte OpenAI API Key prüfen.")
                else:
...
# Results Section
    ...
    <div style='text-align: center; margin: 2rem 0;'>
        ...
        <p style='font-size: 1.25rem; color: #64748b;'>{hotel_city} • {len(df_reviews)} Bewertungen analysiert mit OpenAI GPT-4</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    for insight in insights:
        ...
        # ✅ FIX: Verwende Streamlit Komponenten statt HTML String
        with st.container():
            st.markdown(f"""
    ...
    for key, title, css_class in action_categories:
        ...
        if actions:
            ...
            # ✅ FIX: Use Streamlit components for actions
            for action in actions:
                st.markdown(f"""
                <div style='background: #f8fafc; padding: 1.25rem; border-radius: 8px; margin-bottom: 1rem; border-left: 3px solid #667eea;'>
                    <p style='font-weight: 600; color: #1e293b; margin-bottom: 0.5rem;'>{action.get('action', '')}</p>
                    <p style='color: #64748b; font-size: 0.875rem;'>💡 {action.get('evidence', '')}</p>
                </div>
                """, unsafe_allow_html=True)
    ...
    for idx, review in df_reviews.iterrows():
        # ✅ FIX: Use direct author_url if available, fallback to hotel search
